...
INPUT_FILE <- "Example/data/final_homogroup_results.csv"
CID        <- "Conserved_079"
...
#### Initialize STRINGdb ####
local_db_dir <- "Example/data/stringdb/"
...
# 输入的 ENSP 列表（去掉 9606. 前缀后）
all_ids  <- unique(c(edge_list$from, edge_list$to))
ensp_ids <- sub("9606\\.", "", all_ids)

# 从 EnsDb 提取 ENSP -> gene_name 的映射
# proteins() 返回每个蛋白对应的基因信息
prot_map <- proteins(EnsDb.Hsapiens.v86,
                     columns = c("protein_id", "gene_name")) %>%
  as.data.frame() %>%
  dplyr::select(ensembl_peptide_id = protein_id,
                external_gene_name = gene_name) %>%
  dplyr::filter(ensembl_peptide_id %in% ensp_ids)

# 结果与 biomaRt 输出结构一致（两列 data.frame）
gene_symbols <- prot_map
# 创建顶点数据框
vertices <- data.frame(
  name = all_ids,
...
ggsave(file.path(OUTPUT_DIR, "ppi_centrality_metrics.png"), 
       combined_plot, width = 9, height = 5, dpi = 300)
ggsave(file.path(OUTPUT_DIR, "ppi_centrality_metrics.pdf"), 
       combined_plot, width = 9, height = 5, dpi = 300)
