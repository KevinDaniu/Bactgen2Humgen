...
if unique_clusters:
    print(f"\n统计信息:")
    print(f"  - 最大保守簇: {unique_clusters[0]['gene_count']} 个基因, {unique_clusters[0]['species_count']} 个物种")
    print(f"  - 平均每个保守簇基因数: {sum(c['gene_count'] for c in unique_clusters) / len(unique_clusters):.2f}")
    print(f"  - 平均每个保守簇物种数: {sum(c['species_count'] for c in unique_clusters) / len(unique_clusters):.2f}")
else:
    print("\n警告: 未发现任何保守基因簇")
