...
GFF_DIR="/home/liuzhh/bacteria_cluster/260811_allgenome_ani/result/predicted_cgff"
...
# 获取所有文件列表
echo "正在扫描GFF目录: ${GFF_DIR}" | tee -a "${MAIN_LOG}"

# 获取所有基因组基础名称
all_genomes=()
# 使用绝对路径查找文件
for file in "${GFF_DIR}"/*${FILE_FORMAT}; do
...
if [ ${#all_genomes[@]} -eq 0 ]; then
    echo "错误: 在 ${FNA_DIR} 中没有找到 *.fna 文件" | tee -a "${MAIN_LOG}"
    exit 1
fi

echo "找到以下文件:" | tee -a "${MAIN_LOG}"
...
create_target_db() {
    ...
    # 设置文件路径
    local raw_file="${FNA_DIR}/${target}.fna.gz"
    #PREFIX=$(echo ${target} | cut -d'_' -f1-2)
    PREFIX=$(echo ${target})
    local gff_file="${GFF_DIR}/${PREFIX}${file_format}"
