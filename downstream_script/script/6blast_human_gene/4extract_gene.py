...
parser.add_argument('--blast_dir', type=str,
        default='result/gene2human/blast_results',
        help='Directory containing BLAST result files')
parser.add_argument('--output', type=str,
        default='result/gene2human/blast_results/homology_summary.csv',
...
def parse_blast_line(line, args):
    ...
    if len(subject_parts) >= 6:
        human_gene = subject_parts[5]
        transcript = subject_parts[4]
    else:
        human_gene = "Unknown"
        transcript = "Unknown"
...
for conserved_id, genes_dict in results_dict.items():
    for cluster_gene, hits_dict in genes_dict.items():
        row = {
            ...
            'Human Gene (by Length)': hits_dict['length_hit']['human_gene'],
            'Length (bp) (by Length)': hits_dict['length_hit']['alignment_length'],
            'Identity% (by Length)': hits_dict['length_hit']['perc_identity'],
            'E-value (by Length)': hits_dict['length_hit']['evalue'],
            
            # E-value-based best hit
            'Human Gene (by E-value)': hits_dict['evalue_hit']['human_gene'],
            'Length (bp) (by E-value)': hits_dict['evalue_hit']['alignment_length'],
            'Identity% (by E-value)': hits_dict['evalue_hit']['perc_identity'],
            'E-value (by E-value)': hits_dict['evalue_hit']['evalue'],
            
            # Identity-based best hit
            'Human Gene (by Identity)': hits_dict['identity_hit']['human_gene'],
            'Length (bp) (by Identity)': hits_dict['identity_hit']['alignment_length'],
            'Identity% (by Identity)': hits_dict['identity_hit']['perc_identity'],
            'E-value (by Identity)': hits_dict['identity_hit']['evalue'],
            
            # Comprehensive-based best hit
            'Human Gene (by Comprehensive)': hits_dict['comprehensive_hit']['human_gene'],
            'Length (bp) (by Comprehensive)': hits_dict['comprehensive_hit']['alignment_length'],
            'Identity% (by Comprehensive)': hits_dict['comprehensive_hit']['perc_identity'],
            'E-value (by Comprehensive)': hits_dict['comprehensive_hit']['evalue'],
        }
        
        # Add comprehensive score if available
        if 'comprehensive_score' in hits_dict['comprehensive_hit']:
            row['Comprehensive Score'] = hits_dict['comprehensive_hit']['comprehensive_score']
...
simple_df = summary_df[['Conserved ID', 'Cluster Gene', 
                        'Human Gene (by Length)', 
                        'Human Gene (by E-value)', 
                        'Human Gene (by Identity)', 
                        'Human Gene (by Comprehensive)']].copy()
...
for criteria, col_prefix in [('length', 'by Length'), 
    ...
    for _, row in summary_df.iterrows():
        detailed_rows.append({
            ...
            'human_gene': row[f'Human Gene ({col_prefix})'],
            'alignment_length': row[f'Length (bp) ({col_prefix})'] if f'Length (bp) ({col_prefix})' in row else 0,
            'perc_identity': row[f'Identity% ({col_prefix})'] if f'Identity% ({col_prefix})' in row else 0,
            'evalue': row[f'E-value ({col_prefix})'] if f'E-value ({col_prefix})' in row else 1.0
        })
...
for criteria, col_name in [('Length', 'Human Gene (by Length)'),
                          ('E-value', 'Human Gene (by E-value)'),
                          ('Identity', 'Human Gene (by Identity)'),
                          ('Comprehensive', 'Human Gene (by Comprehensive)')]:
    ...
    if hits_count > 0:
        ...
        if criteria == 'Length':
            print(f"  Average length: {hits_df['Length (bp) (by Length)'].mean():.1f} bp")
        elif criteria == 'E-value':
            print(f"  Median e-value: {hits_df['E-value (by E-value)'].median():.2e}")
        elif criteria == 'Identity':
            print(f"  Average identity: {hits_df['Identity% (by Identity)'].mean():.1f}%")
...
summary_df['All_Same'] = (
    (summary_df['Human Gene (by Length)'] == summary_df['Human Gene (by E-value)']) &
    (summary_df['Human Gene (by Length)'] == summary_df['Human Gene (by Identity)']) &
    (summary_df['Human Gene (by Length)'] == summary_df['Human Gene (by Comprehensive)'])
)
...
if all_same_count < len(summary_df):
    ...
    for _, row in disagreement_df.iterrows():
        ...
        print(f"    Length: {row['Human Gene (by Length)']} (len={row['Length (bp) (by Length)']})")
        print(f"    E-value: {row['Human Gene (by E-value)']} (e={row['E-value (by E-value)']:.2e})")
        print(f"    Identity: {row['Human Gene (by Identity)']} (id={row['Identity% (by Identity)']:.1f}%)")
        print(f"    Comprehensive: {row['Human Gene (by Comprehensive)']}")
